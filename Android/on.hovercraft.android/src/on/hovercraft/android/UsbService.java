...
public class UsbService extends IntentService {
    public static boolean isActive = false; // this is true if the service is up
    // and running
    private boolean accessoryDetached = false; // TODO: true if android
    // accessory is detached

    public static ConnectionState connectionState = ConnectionState.DISCONNECTED; // USB
    // connection
    // state
	...
	// createDriveSignalProtocol(true,true,20);
	// DriveSignals driveSignalRight =
	// createDriveSignalProtocol(true,true,190);
	// Engines engine =
	// createEngineProtocol(driveSignalLeft,driveSignalRight);
	...
	// Log.d(TAG,""+message[x]);
	// }
	// sendCommand(Constants.MOTOR_CONTROL_COMMAND,Constants.TARGET_ADK,
	// message);
	// Log.d(TAG,"Send engine command");

	byte[] message = new byte[1];
	message[0] = Constants.TARGET_BRAIN;
	sendCommand(Constants.I2C_SENSOR_REQ_COMMAND, Constants.TARGET_ADK,
		message);
    }
    ...
    static SensorData createSensorDataProtocol(String type, String desc,
	...
	if (mInputStream != null) {
		...
		byte[] combinedInfoAndPB = new byte[bufferInfo.length
		                                    + bufferPB.length];
		i = 0;
		while (i < combinedInfoAndPB.length) {
		    combinedInfoAndPB[i] = i < bufferInfo.length ? bufferInfo[i]
			    : bufferPB[i - bufferInfo.length];
		    i++;
		}
    ...
    private void handleBrainCommands(byte[] bufferInfo, byte[] bufferPB,
	    byte[] combinedInfoAndPB) {
	switch (bufferInfo[0]) {
	...
	public void onReceive(Context context, Intent intent) {
		...
		String test = "message";
		sendCommand(Constants.BLINKY_ON_COMMAND, Constants.TARGET_ADK,
			test.getBytes());
	    } else if (action.equalsIgnoreCase("sendBlinkyOffCommand")) {
		String test = "message";
		sendCommand(Constants.BLINKY_OFF_COMMAND, Constants.TARGET_ADK,
			test.getBytes());
	    }
	}
    }
